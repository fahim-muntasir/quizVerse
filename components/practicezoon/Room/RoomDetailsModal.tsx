// components/practicezoon/Room/RoomDetailsModal.tsx
...
export default function RoomDetailsModal({
  ...
  const joinRoomHandler = async () => {
    ...
    toast.promise(joinPromise, {
      ...
      error: (err: { data?: { error?: string } }) =>
        err?.data?.error ?? "Failed to join room.",
    });
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="flex items-center gap-2 text-sm text-gray-400 bg-gray-800/50 px-4 py-1.5 rounded-full">
                  <Globe2 size={16} /> {room.language}
                </span>
                <span className="flex items-center gap-2 text-sm text-gray-400 bg-gray-800/50 px-4 py-1.5 rounded-full">
                  <Users size={16} /> 2/{room.maxParticipants}
                </span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {room.members.map((member) => (
                <div key={member.id} className="flex items-center gap-4 p-4 bg-gray-800/50 rounded-lg">
                  <Image src={member.avatar ?? ""} alt={member.name} width={48} height={48} className="w-12 h-12 rounded-full object-cover" />
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gray-900 dark:text-white">{member.name}</span>
                    {member.id === room.hostId && <Crown size={16} className="text-yellow-500" />}
                  </div>
                </div>
              ))}
            </div>
